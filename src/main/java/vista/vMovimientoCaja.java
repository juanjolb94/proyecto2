...
import controlador.cMovimientoCaja;
...
public class vMovimientoCaja extends javax.swing.JInternalFrame implements myInterface {

    private cMovimientoCaja controlador;
    private SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
    private DecimalFormat moneyFormat = new DecimalFormat("#,##0");
    private String tipoMovimiento; // "INGRESO" o "EGRESO"

    public vMovimientoCaja() {
        this("INGRESO"); // Por defecto Ingreso
    }

    public vMovimientoCaja(String tipo) {
        ...
        try {
            controlador = new cMovimientoCaja(this);
    ...
    private void configurarComboTipo() {
        ...
        cmbTipo.addItem("INGRESO");
        cmbTipo.addItem("EGRESO");
        cmbTipo.setSelectedItem(tipoMovimiento);

        // Agregar listener para cambio de tipo
        cmbTipo.addActionListener(e -> {
            String nuevoTipo = (String) cmbTipo.getSelectedItem();
            if (!nuevoTipo.equals(tipoMovimiento)) {
                tipoMovimiento = nuevoTipo;
                actualizarTitulo();
                limpiarCampos(); // Limpiar cuando cambie el tipo
                if (controlador != null) {
                    controlador.cambiarTipo(tipoMovimiento);
                }
            }
        });
    }
    ...
    private void actualizarTitulo() {
        if ("INGRESO".equals(tipoMovimiento)) {
            setTitle("Registro de Ingreso a Caja");
        } else {
    ...
    private void buscarPorId() {
        try {
            ...
            if (!idText.isEmpty()) {
                ...
                if (id > 0) {
                    controlador.cargarMovimiento(id);
                } else {
                    limpiarCampos();
                }
            } else {
    ...
    public void mostrarMovimiento(int id, Date fecha, double monto, String concepto,
            String usuario, boolean anulado, String tipo) {
        ...
        cmbTipo.setEnabled(false); // No permitir cambiar tipo en registros existentes
    }
